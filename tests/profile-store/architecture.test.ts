// Layer-boundary guard for packages/profile-store (M1-002):
// pure modules (errors, fsys, atomic, backup, sanitize, index-types, store) must
// not import Node built-ins — filesystem access flows through the injected Fsys
// and only the wiring module index.ts is allowed to touch Node primitives.
// No private LM Studio path strings may appear anywhere in source.
...
const storeRoot = resolve(workspaceRoot, 'packages/profile-store');
...
const WIRING_MODULES = new Set(['index.ts']);
...
const storeSrcFiles = tsFiles(resolve(storeRoot, 'src'));

describe('profile-store layering (fs access is injected)', () => {
  it('finds the expected source files to scan', () => {
    expect(storeSrcFiles.length).toBeGreaterThanOrEqual(8);
  });

  it('declares exactly one runtime dependency: @lmps/domain', () => {
    const pkg = JSON.parse(readFileSync(join(storeRoot, 'package.json'), 'utf8'));
    expect(Object.keys(pkg.dependencies ?? {}).sort()).toEqual(['@lmps/domain']);
  });

  it.each(storeSrcFiles.map((file) => [relative(workspaceRoot, file), file]))(
    '%s imports no Node built-in outside the wiring module',
    (_label, file) => {
  ...
  it('pure modules never reference Node globals', () => {
    const pureSource = storeSrcFiles
  ...
  it('contains no private LM Studio path strings anywhere', () => {
    const source = storeSrcFiles.map((file) => readFileSync(file, 'utf8')).join('\n');
