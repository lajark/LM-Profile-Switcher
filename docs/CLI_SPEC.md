...
- **M5-009 校准审计落库**：`--yes` 保存时，若基线 `validation.memoryPeakBytes` 为数值，审计行在既有 `at/baselineProfileId/appliedProfileId/taskKind/ruleVersion/confidence/candidateId` 基础上追加 `calibration` 字段（`CalibrationVerdict` 契约：`applied/comparedPeakBytes/estimatedTotalBytes/ratio/degraded/overrunBytes/confidence/note`，由 `@lmps/core` 的 `calibrateEstimate` 计算，估测超实测 >10% 时 `note:'degraded'`）；未实测则无 `calibration` 字段——审计断言数据仅供机器读取，不暴露估算已实测后的静默改写。
