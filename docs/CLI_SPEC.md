...
- **M5-009/M6-002 校准审计落库**：`--yes` 保存时，审计行可追加版本化 `calibration` 字段（`CalibrationVerdict` 保留既有字段，并增加 `calibrationVersion:2`、`evidenceVersion`、`evidenceQuality`、`relation`、`rebenchmarkRequired`）。只有 `BenchmarkMetrics.resourceUsage` 为 complete 的 `ResourceUsageEvidence` v1 才可应用 Total Memory 校准；旧 `memoryPeakBytes` 或 partial/unavailable evidence 返回 `note:'unavailable'`、`rebenchmarkRequired:true`，不提高置信度、不放宽安全余量。历史审计行原样保留，不静默迁移。
- 可信度：`confidence='high'` 仅当 estimate `exact` + 安全余量 safe + 无 `unknown` 能力降级，或实测回流匹配成功（`score.measured=true`，实测背书优先于估算质量）；`rough` 估算永不安全（`unsafe-drop:rough-estimate`），保证「低置信永不静默保存」。
...
- **实测回流排序**：`optimize` 会容错读取 `<LMPS_HOME>/logs/benchmarks.ndjson`（逐行解析，坏行跳过；端口失败降级为纯静态排序），把候选配置（modelKey + quantization + taskType + 六项可调参数逐值相等 + 硬件指纹兼容）与最近一条 completed 且含 decode 的记录匹配。匹配成功的候选：`score.measured=true`、`confidence='high'`（实测背书，可被 `--yes` 保存）、`score.measuredEvidence`（decode/ttft/samples/recordedAt）与 `score.adjustedTotal`（组内 max 归一化的 0.7·decode + 0.3·inverse-TTFT）落入契约；排序规则为「有实测的候选整体排在无实测之前，实测组内按 `adjustedTotal` 降序，无实测组内维持静态 `score.total`，id 升序决胜」。`score.total` 恒为纯静态分（冷启动先验）；人类候选行追加 `optimize.measured` 双语实测段。无 benchmark 日志时行为与旧版完全一致。
...
- 测量规格：`loadMs` = 加载调用 wall-clock；`ttftMs` = 首块 content delta；`generatedTokens` 优先末块 `usage.completion_tokens`，否则 delta 块数；`decodeTokensPerSecond` = generated/(total−ttft)；`prefillTokensPerSecond` = approxPromptTokens/ttft（近似）；旧 `memoryPeakBytes` 保留为弃用的绝对 VRAM used 峰值。M6-002 另在加载前、加载后及每个推理样本后重新探测硬件，以 `resourceUsage.method='host-snapshot-delta'` 记录同步 VRAM/系统 RAM 增量；缺少任一项即 `partial`，不得合成 `totalBytes` 或进行 Total Memory 校准。
- 结果绑定 `hardwareFingerprint` + `lmStudioVersion`/`runtimeVersion` + `modelFileHash`（可空）+ `quantization` + prompt suite id/version；每次运行追加 `logs/benchmarks.ndjson` 审计，**不写 Profile**。结果另带可选 `config` 快照（被测配置的 `profileId` + `gpuOffload`/`contextLength`/`evalBatchSize`/`flashAttention`/`temperature`/`topP`，字段缺失即 defer-to-default），供 optimize 的实测回流做候选匹配（向后兼容可选字段，旧记录仍通过校验）。
- `--yes`（默认不写）：运行完成后 `store.update(id, { validation: { source:'benchmarked', benchmarkId, testedAt, hardwareFingerprint, lmStudioVersion, runtimeVersion, adapterCapabilityVersion, memoryPeakBytes, resourceUsage } })`；其中 `resourceUsage` 原样保存新的同步增量证据，`memoryPeakBytes` 仅为兼容字段。`status !== 'completed'` 永不标记。
...
- **校准回填**：`--yes` 时，仅对 `status:'completed'` 的单项把 `validation{source:'benchmarked', benchmarkId, testedAt, hardwareFingerprint, lmStudioVersion, runtimeVersion, adapterCapabilityVersion, memoryPeakBytes, resourceUsage}` 写入对应 Profile；failed/canceled 永不盖章。
